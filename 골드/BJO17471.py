...
def connect(qu,group):
    check = []
    check.append(qu[0])
    while qu:
        x = qu.popleft()
        for i in con[x]:
            if i in group and i not in check:
                check.append(i)
                qu.append(i)
    ans = 0
    for i in group:
...
for i in range(1,(n//2)+1):
    geri = list(combinations(range(n),i))
    
    for j in range(len(geri)):
        first = geri[j]
        second = []
        for k in range(n):
            if k not in first:
        ...
        qu = deque()
        qu.append(first[0])
        ans1 = connect(qu,first)
        
        qu.append(second[0])
        ans2 = connect(qu,second)
